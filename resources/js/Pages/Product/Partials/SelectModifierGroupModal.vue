...
import EmptyModifierGroup from '@/Pages/Product/Partials/EmptyModifierGroup.vue';
...
const emit = defineEmits(['update:visible', 'update:addGroup']);
...
const loadLazyData = (event) => {
    ...
    try {
        setTimeout(async () => {
            ...
            fetchedGroup.value = results?.data;
...
const addItem = () => {
    emit('update:addGroup', checkedItem.value);
    toast.add({ 
        severity: 'success', 
        summary: trans('public.modifier_group_added_success'), 
        detail: trans('public.modifier_group_added_success_caption'), 
        life: 3000 
    })
    show.value = false;
}
                ...
                >
                    ...
                    >
                        <label :for="`item-${item.id}`" class="flex flex-col items-start gap-1">
                            <div class="font-bold">
                                {{ item.group_name }} ({{ item.item_count }})
                            </div>
                            <div>
                                <template v-for="(data, index) in item.items">
                                    {{ JSON.parse(data.modifier_item_name)[locale] ?? JSON.parse(data.modifier_item_name)['en'] }}{{ index+1 === item.items.length ? '' : ', ' }}
                                </template>
                            </div>
                        </label>
                        <Checkbox 
